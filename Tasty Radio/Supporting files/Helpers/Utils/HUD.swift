...
//  Tasty Radio
//
//  Created by Vitaliy Podolskiy on 11.01.2021.
//  Copyright © 2021 DEVLAB, LLC. All rights reserved.
//

import UIKit

typealias HUDCompletedBlock = () -> Void
...
extension HUD {
    class func show(
        ...
        switch type {
        case .loading:
            headView = UIActivityIndicatorView(style: activityStule)
...
class HUD: NSObject {
    ...
    fileprivate class func getCenter() -> CGPoint {
        if let rv = UIApplication.shared.windows.first?.subviews.first {
    ...
    fileprivate func registerDeviceOrientationNotification() {
        NotificationCenter.default.addObserver(HUD.instance, selector: #selector(HUD.transformWindow(_:)), name: UIDevice.orientationDidChangeNotification, object: nil)
    }
